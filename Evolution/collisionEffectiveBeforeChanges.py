...
class WallCollision(Collision):
	def __init__(self, absoluteTime, circle, isHorizontal):
		Collision.__init__(absoluteTime)
		self.circle = circle
		self.isHorizontal = isHorizontal
	def resolveCollision(self):
		circle.velocity = self.reverseMomentum()
	def reverseMomentum(self):
		if self.isHorizontal == True:
			component = 1;
		else:
			component = 0; 
		velocity = copy.copy(circle.velocity)
		velocity[component] = -1 * velocity[component]
		circle.updateVeloctiy(velocity)
	def report(self,ScircleHash, circleBinTree):
		pass

class CircleCollision(Collision):
	def __init__(self, absoluteTime, circleA, circleB)
		Collision.__init__(absoluteTime)
		self.circleA = circleA
		self.circleB = circleB
	def resolveCollision():
		normalVector = Numerical.distanceBetweenMiddles(self.circleA, self.circleB)
		commonFactor = normalVector/numpy.dot(normalVector, normalVector)
		normalComponentA = numpy.dot(self.circleA.velocity, normalVector)*commonFactor
		normalComponentB = numpy.dot(self.circleB.velocity, normalVector)*commonFactor
		circleANewVelocity = self.circleA.velocity - normalComponentA + normalComponentB
		circleBNewVelocity = self.circleB.velocity - normalComponentB + normalComponentA
		self.circleA.updateVelocity(circleANewVelocity)
		self.circleB.updateVelocity(circleBNewVelocity)
...
class Circle:
	...
	def initializeColor(self, color):
		...
		if color is None:
			...
			return color
	def plotPure(self, absoluteTime = 0):
		"""
		...
		"""
		return pygame.draw.circle(self.surfaceToBlit, self.color, [int(coordinate) for coordinate in (self.position + (absoluteTime - self.lastCollision) * self.velocity)], self.radius)
	def plot(self):
		"""
...
#		self.lastObjectCollidedWith = wallOrCircle
	def expectedWallCollision(self, threshold = 10**-10):
		"""
		...
		for horizontal in wallsHorizontal:
			...
			if solution is not None and solution > threshold:
				solutions.append(WallCollision(solution, self, True))
	...
	def expectedCircleCollision(self, absoluteTime, circle):
...
#			print min(circleCollision)
	...
	def updatePosition(self, absoluteTime): # do it before handling collisions
		self.position = self.position + self.velocity * (self.lastCollision - absoluteTime)
	def updateSpeed(self, newVelocity):
...
class Numerical:
	...
	def solveQuadraticPrune(equation):
		...
		if delta < 0:
			...
			if min(solution) > 0:
				...
				return None
	@staticmethod
	def solveQuadraticPrune_(equation): # deprecated, the only calling solutionPruning
	...
	@staticmethod
	def solutionPruning(twoSolutions): # deprecated
		"""
