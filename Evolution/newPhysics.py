...
class Circle:
	def __init__(self, surfaceToBlit, radius = 10, color = pygame.Color("green"), position = None, angle = None):
		...
		self.bufferFutureSpeed = self.angle
	...
	def giveOffSpeed(self, otherCircle, speed):
		self.bufferFutureSpeed = self.bufferFutureSpeed - speed
		otherCircle.bufferFutureSpeed = otherCircle.angle + speed
	def collisionNormal(self, normalVector):
		shiftVector = numpy.dot(self.bufferFutureSpeed, normalVector)/numpy.dot(normalVector, normalVector)*normalVector
	...
	def move(self, factor = 1):
		if (self.bufferFutureSpeed != self.angle).all():
			self.angle = self.bufferFutureSpeed
		self.wallCollision()
...
#		listOfCircles = [Circle(self.screen) for i in range(10)]
		circleA = Circle(self.screen, position = [200, 200], angle = [1.0, 0])
		circleB = Circle(self.screen, position = [400, 200], angle = [-1.0, 0])
